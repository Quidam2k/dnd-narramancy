...
Also detects D&D Beyond character sheet PDFs and routes to the specialized parser.
"""
...
def parse_pdf_block(path: str, page: int | None = None) -> ParsedCreature:
    ...
    Detects D&D Beyond character sheet PDFs and routes to the specialized parser.
    Otherwise extracts text and delegates to the text block parser.

    Args:
        path: Path to a .pdf file.
        page: 1-based page number to extract. If None, extracts all pages.
    """
    # Check for D&D Beyond character sheet
    from .dndbeyond_pdf import is_dndbeyond_pdf, parse_dndbeyond_pdf
    if is_dndbeyond_pdf(path):
        return parse_dndbeyond_pdf(path)

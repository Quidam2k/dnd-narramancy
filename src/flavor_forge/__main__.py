"""CLI entry point for Flavor Forge.

Usage:
    python -m flavor_forge parse goblin.txt
    python -m flavor_forge parse --open5e goblin
    python -m flavor_forge parse --open5e "adult-red-dragon" --json
    python -m flavor_forge parse goblin.txt --context "These goblins serve a hobgoblin warlord"

    python -m flavor_forge generate --open5e goblin --style dramatic --variations 3
    python -m flavor_forge generate --open5e goblin --provider ollama
    python -m flavor_forge generate --open5e goblin --providers gemini,ollama --compare
    ...
    python -m flavor_forge export results.json --format foundry --output goblin-tables.json
    python -m flavor_forge export results.json --format tokensays --output goblin-sayings.json
"""
...
def _run_batch(args, config):
    ...
    # Collect creature files (.txt and .json, skip flavor.txt)
    files = sorted([
        fn for fn in os.listdir(batch_dir)
        if fn.lower().endswith(('.txt', '.json'))
        and fn.lower() != 'flavor.txt'
    ])

    if not files:
        print(f"No .txt or .json files found in {batch_dir}", file=sys.stderr)
        return 1

    print(f"Found {len(files)} creature files", file=sys.stderr)
    ...
    for fn in files:
        filepath = os.path.join(batch_dir, fn)
        print(f"\nProcessing {fn}...", file=sys.stderr)

        try:
            if fn.lower().endswith('.json'):
                with open(filepath, 'r') as f:
                    data = json.load(f)
                creature = _dict_to_creature(data)
            else:
                creature = parse_stat_block(filepath)
...
def main():
    parser = argparse.ArgumentParser(
        prog='flavor_forge',
        description='D&D Flavor Forge — vivid narration for abilities and actions',
    )
    ...
    # webui subcommand
    webui_cmd = subparsers.add_parser('webui', help='Launch the Gradio web UI')
    webui_cmd.add_argument('--port', type=int, default=7870, help='Port to serve on (default: 7870)')
    webui_cmd.add_argument('--share', action='store_true', help='Create a public Gradio share link')
